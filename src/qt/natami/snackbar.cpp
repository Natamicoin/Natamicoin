...
#include "qt/natami/forms/ui_snackbar.h"
#include "qt/natami/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(NATAMIGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &NATAMIGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
