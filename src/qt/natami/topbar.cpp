...
#include "qt/natami/forms/ui_topbar.h"
#include "qt/natami/loadingdialog.h"
#include "qt/natami/lockunlock.h"
#include "qt/natami/qtutils.h"
#include "qt/natami/receivedialog.h"
...
TopBar::TopBar(NATAMIGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                        ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelPendingPiv, ui->labelImmaturePiv, ui->labelAvailablePiv,
                       ui->labelLockedPiv, ui->labelMasternodeCount, ui->labelCollateralNata,
...
//    ui->pushButtonFAQ->setButtonText(tr("FAQ"));
    ...
    ui->pushButtonConf->setButtonText("natami.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open natami.conf with default application"));
}
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralNata->setText(GUIUtil::formatBalance(CMasternode::GetMasternodeNodeCollateral(chainActive.Tip()->nHeight), nDisplayUnit));
}
