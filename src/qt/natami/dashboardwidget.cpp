...
#include "qt/natami/forms/ui_dashboardwidget.h"
#include "qt/natami/sendconfirmdialog.h"
#include "qt/natami/txrow.h"
...
DashboardWidget::DashboardWidget(NATAMIGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of NATA earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 NATA");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &NATAMIGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
 ...
 //       connect(ui->pushImgEmpty, &QPushButton::clicked, window, &NATAMIGUI::openFAQ);
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("NATA")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair NATA, zNATA
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(154,202,60));
	set1->setColor(QColor(84,160,47));
