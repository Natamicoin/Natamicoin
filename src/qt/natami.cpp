...
#include "config/natami-config.h"
#endif
...
#include "qt/natami/splash.h"
#include "qt/natami/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("natami-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in natami.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in natami.qrc)
...
/** Class encapsulating Natami Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (NATAMIGUI)
...
{
    window = new NATAMIGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &NATAMIGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &NATAMIGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(NATAMIGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(NATAMIGUI::DEFAULT_WALLET);
        ...
        // Natami: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NATAMIGUI::handlePaymentRequest);
        connect(window, &NATAMIGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Natami can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(natami_locale);
    Q_INIT_RESOURCE(natami);
    ...
    /// 6. Determine availability of data directory and parse natami.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Natami"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Natami"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // natami: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Natami didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
