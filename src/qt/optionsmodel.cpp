...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeNatamiAmount") || reset)
        SoftSetArg("-anonymizenatamiamount", settings.value("nAnonymizeNatamiAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in natami.cpp during shotdown to also remove the window positions
