...
extern UniValue DoZnataSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str);
...
extern UniValue setznataseed(const JSONRPCRequest& request);
extern UniValue getznataseed(const JSONRPCRequest& request);
extern UniValue generatemintlist(const JSONRPCRequest& request);
extern UniValue searchdznata(const JSONRPCRequest& request);
extern UniValue dznatastate(const JSONRPCRequest& request);
