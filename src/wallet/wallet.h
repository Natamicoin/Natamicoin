...
#include "znata/znatamodule.h"
#include "znata/znatawallet.h"
#include "znata/znatatracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_NATAMI = 61000, // inherited version..
...
// Possible states for zNATA send
enum ZerocoinSpendStatus {
    ZNATA_SPEND_OKAY = 0,                            // No error
    ZNATA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNATA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNATA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNATA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNATA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNATA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNATA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNATA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNATA_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZNATA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNATA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNATA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNATA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNATA_SPENT_USED_ZNATA = 14,                      // Coin has already been spend
    ZNATA_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZNATA_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void doZNataRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZNATA);
    ...
    bool CreateZNATAOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zNATA wallet
    std::unique_ptr<CzNATATracker> znataTracker{nullptr};
    void setZWallet(CzNATAWallet* zwallet);
    CzNATAWallet* getZWallet();
