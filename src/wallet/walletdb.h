...
public:
    ...
    bool WriteZNATASeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZNATASeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZNATASeed_deprecated(uint256& seed);
    bool EraseZNATASeed();
    bool EraseZNATASeed_deprecated();

    bool WriteZNATACount(const uint32_t& nCount);
    bool ReadZNATACount(uint32_t& nCount);
