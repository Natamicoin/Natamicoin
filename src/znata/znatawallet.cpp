...
CzNATAWallet::CzNATAWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zNATA seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNATASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNATASeed_deprecated()) {
                    LogPrintf("%s: Updated zNATA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zNATA seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zNATA wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNATAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNATACount(nCountLastUsed);
    else if (!walletdb.ReadZNATACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNATAWallet::Lock()
{
    seedMaster.SetNull();
}

void CzNATAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNATAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNATA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNATAWallet::LoadMintPoolFromDB()
{
...
void CzNATAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNATAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNATAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->znataTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNATAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNATA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zNATATracker which also adds to database
    wallet->znataTracker->Add(dMint, true);
...
void CzNATAWallet::SeedToZNATA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzNATAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNATAWallet::GenerateDeterministicZNATA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNATAWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNATAWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNATAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
