...
bool CLegacyZNataStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZNATAStake from non zc-spend", __func__);
    ...
    *this = CLegacyZNataStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zNATA stake origin", __func__);

    // All good
    return true;
}

CLegacyZNataStake::CLegacyZNataStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZNataStake::GetIndexFrom()
{
...
CAmount CLegacyZNataStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZNataStake::GetUniqueness() const
{
...
bool CLegacyZNataStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zNATA stake block: height %d outside range", __func__, nHeight);
