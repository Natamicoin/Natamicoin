...
#include "znata/znatawallet.h"


CzNATATracker::CzNATATracker(CWallet* parent)
{
    this->wallet = parent;
...
CzNATATracker::~CzNATATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNATATracker::Init()
{
...
bool CzNATATracker::Archive(CMintMeta& meta)
{
...
bool CzNATATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNATATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNATATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNATATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzNATATracker::GetSerialHashes()
{
...
CAmount CzNATATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNATATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNATATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNATATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNATATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNATATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNATATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNATATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNATATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNATATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzNATATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNATAWallet* zNATAWallet)
{
    bool iszNATAWalletInitialized = (nullptr != zNATAWallet);
    CMintMeta meta;
    ...
    if (!iszNATAWalletInitialized)
        zNATAWallet = new CzNATAWallet(wallet);
    meta.isSeedCorrect = zNATAWallet->CheckSeed(dMint);
    if (!iszNATAWalletInitialized)
        delete zNATAWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNATATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNATATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNATATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNATATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNATATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNATATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dznata from DB\n", __func__, listDeterministicDB.size());
    }
