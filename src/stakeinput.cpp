...
bool CNataStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CNATAStake from zerocoin spend", __func__);
...
bool CNataStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CNataStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CNataStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CNataStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CNataStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CNataStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CNataStake::GetUniqueness() const
{
    //The unique identifier for a NATA stake is the outpoint
...
CBlockIndex* CNataStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CNataStake::ContextCheck(int nHeight, uint32_t nTime)
{
