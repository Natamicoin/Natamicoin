...
class ZerocoinSpendTest(NatamiTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, znata_bal, nata_bal):
            znata_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], znata_bal)
            nata_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], nata_bal)
            return znata_bal, nata_bal
        ...
        znata_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(znata_balance, 6666)
        ...
        znata_balance, balance = check_balances(denom_2, znata_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        znata_balance, balance = check_balances(denom_3, znata_balance, balance)
        ...
        _, _ = check_balances(denom_1, znata_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
