...
from test_framework.test_framework import NatamiTestFramework
...
class ReorgStakeTest(NatamiTestFramework):
    ...
    def check_money_supply(self, expected_nata, expected_znata):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected NATA and zNATA supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_nata))
            for denom in node['zNATAsupply']:
                assert_equal(node['zNATAsupply'][denom], DecimalAmt(expected_znata[denom]))
    ...
    def run_test(self):
        ...
        # Check NATA and zNATA supply at the beginning
        # ------------------------------------------
        # zNATA supply: 2 coins for each denomination
        expected_znata_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # NATA supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_znata_supply)
        ...
        # Verify that NATA and zNATA supplies were properly updated after the spends and reorgs
        self.log.info("Check NATA and zNATA supply...")
        ...
        expected_znata_supply[str(spent_coin_0)] -= spent_coin_0
        expected_znata_supply[str(spent_coin_1)] -= spent_coin_1
        expected_znata_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_znata_supply)
